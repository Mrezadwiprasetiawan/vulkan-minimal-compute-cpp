...
int main() {
  ...
  vector<float> inputs;
  for(int i=0;i<WIDTH;i++) inputs.push_back(dis(gen));
  ...
  auto queueFamProp=find_if(queueFamProps.begin(),queueFamProps.end(),
  ...
  //map memori lalu isi
  float* inBuffPtr=static_cast<float*>(dev.mapMemory(inBuffMem,0,buffSize));
  //salin memori
  ...
  DescriptorSetLayoutCreateInfo descSetLayoutInfo(DescriptorSetLayoutCreateFlags(),descSetLayoutBind);
  DescriptorSetLayout descSetLayout=dev.createDescriptorSetLayout(descSetLayoutInfo);

  // membuat pipeline layout
  PipelineLayoutCreateInfo compPipeLayoutInfo(PipelineLayoutCreateFlags(), descSetLayout);
  ...
  DescriptorPoolCreateInfo descPoolInfo(DescriptorPoolCreateFlags(),1,descPoolSize);
  ...
  vector<DescriptorSet> descSets=dev.allocateDescriptorSets(descSetsAllocInfo);
  ...
  const vector<WriteDescriptorSet> writeDescSets{
    {descSet,0,0,1,DescriptorType::eStorageBuffer,nullptr,&descInBuffInfo},
    {descSet,1,0,1,DescriptorType::eStorageBuffer,nullptr,&descOutBuffInfo}
  };
  ...
  cmdBuff.bindDescriptorSets(PipelineBindPoint::eCompute,
      compPipeLayout,0,{descSet},{});
  ...
  cout<<"Input= ["<<endl;
  for(uint32_t i=0;i<WIDTH;++i) cout<<inBuffPtr[i]<<","<<endl;
  cout<<"]"<<endl<<"Output= [";
  for(uint32_t i=0;i<WIDTH;++i) cout<<outBuffPtr[i]<<","<<endl;
  cout<<"]"<<endl;
